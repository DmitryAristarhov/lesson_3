...
for i in text:
    temp.add(i)
temp.remove(' ') # Пробелы, понятно что есть, и нас не интересуют
temp.remove('\n') # Переносы строк - тоже
ltemp = list(temp)
ltemp.sort()
...
print(punctuation) # Длинного "тире" '—' нет! И таких кавычек '«»'
add = '—«»0123456789' # Цифры нам тоже не нужны
punct = set(punctuation)
for i in add:
    punct.add(i)
punct.add(i)
punct.remove('-') # А дефис пригодится
ltemp = list(punct)
ltemp.sort()
print(*ltemp, sep='')
# Итого:
...
# Заменим символы пунктуации пробелами
for i in punctuation:
    text = text.replace(i, ' ')

# И запишем в файл
with open('t1.txt', mode = 'wt', encoding = 'utf-8') as f:
    f.write(text)

# разберем на слова в список
words = text.split()

# И запишем в файл через ";"
with open('t2.txt', mode = 'wt', encoding = 'utf-8') as f:
    for i in words:
        f.write(i)
        f.write(';')

# Приведем все слова к нижнему регистру
temp = list(map(lambda x: x.lower(), words)) # Если нужно обязательно применить map()
list_text = [i.lower() for i in words] # Но так мне больше нравится ;)
print(list_text)
############################################################################
